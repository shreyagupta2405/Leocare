...
function EmpowerHome() {
    ...
    const addEventToStore = async (data, url) => {
        try {
            await empowerHomeService.addEvent(
                {
    ...
    const updateEventToStore = async (data, url) => {
        ...
        try {
            await empowerHomeService.updateEvent(
                editId,
                {
                    "url": url,
                    "content": data?.content,
    ...
    const getAllEventFromStore = async () => {
        try {
            const data = await empowerHomeService.getAllEvents()
            let arr = []
            data.forEach((doc) => {
    ...
    const deleteEvent = async (id) => {
        await empowerHomeService.deleteEvents(id);
        getAllEventFromStore();
    }
    ...
    const editHandler = async (id) => {
        setEdit(true);
        setEditId(id);
        const docSnap = await empowerHomeService.getEvent(id);
    ...
    <div class="lg:flex">
            <div class="lg:w-1/2 xl:max-w-screen-sm ">
                <div class="mt-2 px-2 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
                    ...
                    xl:text-bold ">Women Empowerment Home</h2>
