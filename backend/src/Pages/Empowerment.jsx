...
import empowermentService from '../api/empowerment.service'
...
function Empowerment() {
    ...
    const addEventToStore = async (data, url) => {
        try {
            await empowermentService.addEvent(
                {
    ...
    const updateEventToStore = async (data, url) => {
        ...
        try {
            await empowermentService.updateEvent(
                editId,
                {
                    "content": data?.content,
                    "date": data?.postDate,
    ...
    const getAllEventFromStore = async () => {
        try {
            const data = await empowermentService.getAllEvents()
            let arr = []
            data.forEach((doc) => {
    ...
    const deleteEvent = async (id) => {
        await empowermentService.deleteEvents(id);
        getAllEventFromStore();
    }
    ...
    const editHandler = async (id) => {
        setEdit(true);
        setEditId(id);
        const docSnap = await empowermentService.getEvent(id);
    ...
    <div class="lg:flex">
            <div class="lg:w-1/2 xl:max-w-screen-sm ">
                <div class="mt-2 px-2 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
                    ...
                    xl:text-bold ">Women Empowerment</h2>
